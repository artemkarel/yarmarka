...
TOPICS = [
    # посетительские рубрики берём целиком: любая выставка-продажа с трафиком —
    # потенциальная точка торговли
    ("fairs", None),          # ярмарки
    ("customergoods", None),  # товары народного потребления, подарки
    ("moda", None),           # текстиль, мода, косметика, ювелирка
    ("cultureart", None),     # ремёсла, антиквариат, православные ярмарки
    ("pets", None),           # зоовыставки
    ...
    ("agriculture", re.compile(  # «Сельское хозяйство, пищевая промышленность»
        r"(?i)продукт|пищев|продовол|кондитер|сладо|мёд|медов|фермер|агро|урожа"
        r"|ярмарк|гастро|напитк|сыр|органик|эко|цвет|флор|сад\b|садов|огород|дач")),
    ("sport", re.compile(  # посетительские: охота-рыбалка, отдых, авто-шоу
        r"(?i)охот|рыбол|отдых|туриз|мото|интеравто|шоу")),
]
